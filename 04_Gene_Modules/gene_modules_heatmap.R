...
data_subset_norm <- t(apply(mat.norm.bin, 1, cal_z_score))
pheatmap(data_subset_norm, annotation_colors = df$n.genes,
         cutree_cols = 4,
         cutree_rows = 4,
         legend = T)

##Plot a heatmap with modules vs tissue.groups
library(pheatmap)
pdf(file = "~/Box/zfext/genemodule_celltype_curated/2022-05 modules on mama/figures/module_expression_across_identity.super_shared_new_annot.pdf", width = 72, height = 60)
pheatmap(mat.norm.bin, 
...
##Create a dotplot for the ciliary genes - Figure S4A
...
##For epithelial dotplot, we need to assign different idents to mama - Figure 2C
...
##Let's do the a figure for the lysosomal degradation pathway - Figure S4B
...
##Also plot the members of the SLC pathway and the megalin expression pathway - Figure 2B
...
##Plot a dotplot for the mucin genes and the associated O-glycosylation enzymes  - Figure 2D
sample <- "ionocytes"
